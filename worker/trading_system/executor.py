...
def validate_intent(
    ...
    if intent.size * reference > MAX_LIVE_NOTIONAL:
        ...
        with self.database.connect() as connection:
            ...
            if existing:
                ...
                connection.execute(
                    ...
                    SET exchange_order_id = %s, state = %s, detail = %s::jsonb
                    WHERE client_order_id = %s
                    """,
                    (
                        recovered["ordId"],
            ...
            connection.execute(
                ...
                (
                    ...
                    intent.size,
                    intent.price,
                    json.dumps({"riskDecisionId": intent.risk_decision_id}),
                ),
            )
            connection.commit()
            order = {
                ...
                "side": intent.action,
                "posSide": "long",
                "ordType": "limit" if intent.price is not None else "market",
            ...
            connection.execute(
                """
                UPDATE execution_audit
                SET exchange_order_id = %s, state = 'submitted', detail = %s::jsonb
                WHERE client_order_id = %s
                """,
                (
                    result["ordId"],
